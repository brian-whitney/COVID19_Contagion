...
class World:
    def __init__(self, population, sizex, sizey):
        ...
        #self.incubation_period = 14
        
        self.sizex = sizex
        self.sizey = sizey
        self.health = ("susceptible", "infectious", "recovered")
                        ...
                        if status2 == "susceptible":
                            if distance <= self.contact_distance:
                                ...
                                if number <= self.transmission_rate:
                                    self.people_update[index2][3] = "infectious"
...
for k in range(30):
    ...
    world.update()
    stats.append([k, num_susceptible, num_infectious, num_recovered])
...
plt.plot(statsarray[:,0], statsarray[:,2], label="Infectious", color="purple")
plt.plot(statsarray[:,0], statsarray[:,3], label="Recovered", color="green")
plt.legend()
plt.grid(color = "lightblue", alpha = 0.5)
plt.xlabel("Days")
